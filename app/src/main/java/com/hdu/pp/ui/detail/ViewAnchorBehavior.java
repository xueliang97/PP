...
/**
 * CoordinatorLayout里控件摆放位置
 */
public class ViewAnchorBehavior extends CoordinatorLayout.Behavior<View> {//<View>指behavior被应用的View
    ...
    public ViewAnchorBehavior(Context context, AttributeSet attributeSet){
        TypedArray array = context.obtainStyledAttributes(attributeSet, R.styleable.view_anchor_behavior);
        int anchorId = array.getResourceId(R.styleable.view_anchor_behavior_anchorId, 0);
        array.recycle();
    ...
    //CoordinatorLayout 就不会再次测量child了。会使用咱们给的测量的值去摆放view的位置
    @Override
